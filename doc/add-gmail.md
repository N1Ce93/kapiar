...
- Проверяются только письма с меткой `UNREAD`.
- Письма из `SPAM` проверяются наравне со входящими.
- Письма с метками `SENT`, `DRAFT` и `TRASH` пропускаются.
...
- Успешно обработанные письма в базе не сохраняются.
...
Scheduler каждую минуту запрашивает dispatch, но `CheckGmailJob` является уникальной задачей. Redis-lock дополнительно не позволяет одновременно запустить проверку из cron, другого worker или команды `gmail:check`.
